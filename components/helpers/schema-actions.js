define('schema-actions', [
    ...
    './post-via-form.js',
    './dialogs.js'
], (ajax, createElement, postViaForm, dialogs) => ({
    action(action, dlg, container, data = {}) {

        const request = ajax.execute;
        const schemaProps = (action && action.schema) ? action.schema.properties : {};
                ...
                dataType: 'json'
                    ...
                    .then(resData => {
                        ...
                        if (resData) {
                            ...
                            const actions = this.getActionsFromData(resData);

                            if (actions.length === 1) {
                                return this.action(actions[0], dlg, container);
                            }
                        }
