 ...
 */
define('validate-against-schema', ['./get-property-by-path.js'], (getPropertyByPath) => {
    const validateAgainstSchema = function (schema, prop, value) {
        ...
        if (schemaItem) {
            ...
            if (value && schemaItem.minimum && parseFloat(value) < schemaItem.minimum) {
                return `The value must not be lower than ${schemaItem.minimum}`;
            }

            if (value && schemaItem.maximum && parseFloat(value) > schemaItem.maximum) {
